...
Before publishing, replace `INVITE_URL_PLACEHOLDER` in `index.html` with the real OAuth2 invite URL, and `GITHUB_REPO_PLACEHOLDER` with the bot repo link. To deploy: import this repo into Vercel via the dashboard and accept the defaults — it's a static folder, so no configuration is needed.
